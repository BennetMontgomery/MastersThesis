...
def plot_validate():
    for i in range(len(configs)):
        ...
        for network in networks:
            reward = ego.validate(configs[i], validation_folder, npcs=npcs[i], network=network)

            rewards.append(reward)

        plt.plot(domain, rewards)
        plt.title(f"{configs[i]} performance over time")
        plt.savefig(f"./{configs[i]}.png")
        plt.close()

def graphically_validate(confg_idx=0, network="2022-11-14 19:05:20.309803_100"):
    reward = ego.validate(configs[confg_idx], validation_folder, npcs=npcs[confg_idx], network=network, graphical_mode=True)
